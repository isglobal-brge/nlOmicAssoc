...
plot.nlAssoc <- function(res, top.probes=10, pdf.name=NULL)
{
  ...
  res.f <- filter.nlAssoc(res, threshold = 0.9999) #very relaxing threshold to get something
  res.f
  sorted.probes<-res.f$table$probe[1:top.probes]
  sorted.vars<-res.f$selected_vars[1:top.probes]

  data_m <- res$set.def
  vars_df <- res$vars_df.def
  ...
  if (model=="ewas"){
    ...
    for (i in 1:top.probes){
      ...
      p <- qplot(x=data_m[probe,],y=mod.pred, xlab = paste("probe",probe), ylab = "predicted")
